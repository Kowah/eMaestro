...
public class MaestroBDD {
    ...
    public MaestroBDD(Context context){
        baseSQLite = new DatabaseHelper(context,"eMaestroBDD",null,1);
    }
    ...
    public void insertMusique(int id, String nom, int nbMesures){
        ContentValues values = new ContentValues();
    ...
    public ArrayList<Pair<Integer,String>> selectAllMusique(){
        ArrayList<Pair<Integer,String>> liste = new ArrayList<>();

        String query = "SELECT id_musique, name FROM Musique";
        Cursor cursor = bdd.rawQuery(query,null);
        ...
        if(cursor.moveToFirst()){
            do {
                ...
                liste.add(new Pair(new Integer(id),nom));
